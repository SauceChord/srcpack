...
describe("LuaScanner", function()
    describe("scanTokens()", function()
        it("produces tokens from sourcecode", function()
            scans_sourcecode("print(i * 3) -- Lorem ipsum was here")
            expects_output(T.Identifier, "print")
            expects_output(T.OpenBracket, "(")
            expects_output(T.Identifier, "i")
            expects_output(T.Multiply, "*")
            expects_output(T.Number, "3", 3)
            expects_output(T.CloseBracket, ")")
            expects_output(T.Comment, "-- Lorem ipsum was here")
            expects_output(T.EndOfStream, nil)
        end)
        it("produces end of stream token", function()
            scans_sourcecode("")
            expects_output(T.EndOfStream)
        end)
        ...
        describe("types", function()
            ...
            it("handles malformed integers", function()
                scans_sourcecode("0a")
                expects_output(T.Error, "0a")
                expects_output(T.EndOfStream)
            end)
            it("handles malformed decimals", function()
                scans_sourcecode("0.0a")
                expects_output(T.Error, "0.0a")
                expects_output(T.EndOfStream)
            end)
            it("handles malformed decimals with multiple decimal separators in it", function()
                scans_sourcecode("0.0.0")
                expects_output(T.Error, "0.0.0")
                expects_output(T.EndOfStream)
            end)
            it("handles malformed hexadecimals", function()
                scans_sourcecode("0x0g")
                expects_output(T.Error, "0x0g")
                expects_output(T.EndOfStream)
            end)
            it("handles '' strings", function()
                scans_sourcecode("'Lorem'")
                expects_output(T.String, "'Lorem'", "Lorem")
                expects_output(T.EndOfStream)
            end)
            it('handles "" strings', function()
                scans_sourcecode('"Lorem"')
            ...
            it("handles strings with escape characters", function()
                ...
                expects_output(T.String, "'Lorem \\' ipsum'", "Lorem \\' ipsum")
                expects_output(T.String, "'dolor \\\" sit'", "dolor \\\" sit")
                expects_output(T.EndOfStream)
            end)
        end)
        ...
        describe("keywords", function()
            it("handles and", function()
                scans_sourcecode("and")
                expects_output(T.And, "and", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles break", function()
                scans_sourcecode("break")
                expects_output(T.Break, "break", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles do", function()
                scans_sourcecode("do")
                expects_output(T.Do, "do", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles else", function()
                scans_sourcecode("else")
                expects_output(T.Else, "else", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles elseif", function()
                scans_sourcecode("elseif")
                expects_output(T.ElseIf, "elseif", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles end", function()
                scans_sourcecode("end")
                expects_output(T.End, "end", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles false", function()
                scans_sourcecode("false")
                expects_output(T.False, "false", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles for", function()
                scans_sourcecode("for")
                expects_output(T.For, "for", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles function", function()
                scans_sourcecode("function")
                expects_output(T.Function, "function", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles goto", function()
                scans_sourcecode("goto")
                expects_output(T.Goto, "goto", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles if", function()
                scans_sourcecode("if")
                expects_output(T.If, "if", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles in", function()
                scans_sourcecode("in")
                expects_output(T.In, "in", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles local", function()
                scans_sourcecode("local")
                expects_output(T.Local, "local", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles nil", function()
                scans_sourcecode("nil")
                expects_output(T.Nil, "nil", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles not", function()
                scans_sourcecode("not")
                expects_output(T.Not, "not", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles or", function()
                scans_sourcecode("or")
                expects_output(T.Or, "or", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles repeat", function()
                scans_sourcecode("repeat")
                expects_output(T.Repeat, "repeat", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles return", function()
                scans_sourcecode("return")
                expects_output(T.Return, "return", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles then", function()
                scans_sourcecode("then")
                expects_output(T.Then, "then", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles true", function()
                scans_sourcecode("true")
                expects_output(T.True, "true", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles until", function()
                scans_sourcecode("until")
                expects_output(T.Until, "until", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles while", function()
                scans_sourcecode("while")
                expects_output(T.While, "while", nil, 1)
                expects_output(T.EndOfStream)
            end)
        end)
        describe("identifiers", function()
            it("handles text", function()
                scans_sourcecode("lorem")
                expects_output(T.Identifier, "lorem", nil, 1)
                expects_output(T.EndOfStream)
            end)
            it("handles underscore", function()
                scans_sourcecode("_lorem")
                expects_output(T.Identifier, "_lorem", nil, 1)
                expects_output(T.EndOfStream)
            end)
        end)
