    ...
    describe("scanTokens()", function()
        describe("with valid inputs", function()
            ...
            it("produces 10 tokens from this source", function()
                ...
                expects(T.Comment, "--Lorem *", nil, 2)
                expects(T.String, "'ipsum'", "ipsum", 3)
                expects(T.String, "[[dolor\nsit]]", "dolor\nsit", 3)
            ...
            describe("comments", function()
                it("handles -- comments", function()
                    scans("--Lorem\n--ipsum")
                    expects(T.Comment, "--Lorem", nil, 1)
                    expects(T.Comment, "--ipsum", nil, 2)
                    expects(T.EndOfStream, nil, nil, 2)
                end)
                it("handles --[[]] comments", function()
                    scans("--[[Lorem\nipsum]]")
                    expects(T.MultilineComment, "--[[Lorem\nipsum]]", nil, 1)
                    expects(T.EndOfStream, nil, nil, 2)
                end)
            end)
            ...
            describe("types", function()
                ...
                it("handles exponentiated numbers", function()
                    scans("1E2 1e+2 1e-2 0x1P8 0x1p+8 0x1p-1")
                    -- Usual exponent is e or E
                    expects(T.Number, "1E2", 100, 1)
                    expects(T.Number, "1e+2", 100, 1)
                    expects(T.Number, "1e-2", .01, 1)
                    -- Hexadecimal exponent is p or P
                    expects(T.Number, "0x1P8", 256, 1)
                    expects(T.Number, "0x1p+8", 256, 1)
                    expects(T.Number, "0x1p-1", .5, 1)
                    expects(T.EndOfStream, nil, nil, 1)
                end)
                it("handles '' strings", function()
                    scans("'Lorem'")
                    expects(T.String, "'Lorem'", "Lorem", 1)
                    expects(T.EndOfStream, nil, nil, 1)
                end)
                it('handles "" strings', function()
                    scans('"Lorem"')
                ...
                it("handles [[]] strings", function()
                    scans("[[Lorem\nipsum]]")
                    expects(T.String, "[[Lorem\nipsum]]", "Lorem\nipsum", 1)
                    expects(T.EndOfStream, nil, nil, 2)
                end)
                it("handles strings with escape characters", function()
                    scans("'Lorem\\'ipsum' 'dolor' [[sit\\]]]")
                    expects(T.String, "'Lorem\\'ipsum'", "Lorem\\'ipsum", 1)
                    expects(T.String, "'dolor'", "dolor", 1)
                    expects(T.String, "[[sit\\]]]", "sit\\]", 1) -- odd case?
                    expects(T.EndOfStream, nil, nil, 1)
                end)
            end)
