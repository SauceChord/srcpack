...
describe("LuaScanner", function()
    ...
    describe("scanner:scanToken()", function()
        ...
        it("returns DIV_FLOOR", function()
            local scanner = LuaScanner.new("//")
            assert.are.same("DIV_FLOOR // nil 1", scanner:scanToken():tostring())
        end)
        it("returns BNOT", function()
            local scanner = LuaScanner.new("~")
            assert.are.same("BNOT ~ nil 1", scanner:scanToken():tostring())
        end)
        it("returns NOT_EQUAL", function()
            local scanner = LuaScanner.new("~=")
            assert.are.same("NOT_EQUAL ~= nil 1", scanner:scanToken():tostring())
        end)
